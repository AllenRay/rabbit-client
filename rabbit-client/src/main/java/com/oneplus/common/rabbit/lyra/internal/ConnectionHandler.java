...
import com.oneplus.common.rabbit.lyra.internal.util.ArrayListMultiMap;
import com.oneplus.common.rabbit.lyra.internal.util.Collections;
import com.oneplus.common.rabbit.lyra.internal.util.Exceptions;
...
public class ConnectionHandler extends RetryableResource implements InvocationHandler {
  ...
  public ConnectionHandler(ConnectionOptions options, Config config) throws IOException {
    ...
    this.connectionName =
        options.getName() == null ? String.format("cxn-%s", CONNECTION_COUNTER.incrementAndGet())
            : options.getName();
    //if consumer executor has no set,will use default fixed(5) thread pool.
    consumerThreadPool =
        options.getConsumerExecutor() == null ? Executors.newFixedThreadPool(5,new NamedThreadFactory(
  ...
  public Object invoke(Object ignored, final Method method, final Object[] args) throws Throwable {
    ...
    try {
      ...
      if ("createProducerChannel".equals(method.getName())) {
